...
import { Loader2, X, Download, Share2, MessageCircle, Crown, Plus, ChevronDown } from "lucide-react";
...
export default function CrearAnuncioIndumentariaPage() {
  const { data: session } = useSession();
  const [imageFiles, setImageFiles] = useState<{ top?: File; bottom?: File }>({});
  const [descriptions, setDescriptions] = useState<{ top: string; bottom: string }>({
    top: "",
    bottom: "",
  });
  const [aiGenerated, setAiGenerated] = useState<{ top: boolean; bottom: boolean }>({
    top: false,
    bottom: false,
  });
  ...
  const generateSingleImage = async (
    ...
    const response = await fetch("/api/generate", {
      ...
      body: JSON.stringify({
        images: [topUrl, bottomUrl],
        modelType,
        gender,
        bodyType,
        topDescription: descriptions.top,
        bottomDescription: descriptions.bottom,
        size: selectedSize,
      }),
    });
    ...
    const images = await pollForResult(requestId, "anuncios", userId);
    return images && images.length > 0 ? images[0] : null;
  };

  const handleGenerate = async () => {
    if (!imageFiles.top || !imageFiles.bottom) {
      alert("Sube ambas imágenes antes de generar");
      return;
    }
    ...
    try {
      ...
      const [topUrl, bottomUrl] = await Promise.all([
        uploadImage("anuncios", "upload", userId, imageFiles.top!),
        uploadImage("anuncios", "upload", userId, imageFiles.bottom!),
      ]);
      ...
      const prompt = `Generate a ${bodyTypeText}${modelType} young ${genderText} model wearing the upper garment ${descriptions.top || ""} and the bottom garment ${descriptions.bottom || ""}, white background, full body shot`;

      // Generar múltiples imágenes en paralelo
      const imagePromises = Array.from({ length: imageCount }, () =>
        generateSingleImage(topUrl, bottomUrl, modelType, userId)
      );
  ...
  const handleImageSelect = (file: File, type: "top" | "bottom") => {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-center">
        <p className="text-sm font-medium text-amber-800">Funcionalidad BETA</p>
        <p className="text-sm text-amber-700">Funcionalidad todavía en prueba, puede cometer errores o tardar más en brindar resultados.</p>
      </div>
      ...
      <div className="flex gap-6">
        <div className="grid flex-1 grid-cols-1 gap-6 md:grid-cols-2">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-gray-700">Parte superior</label>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">Generado por IA</span>
                <Switch
                  checked={aiGenerated.top}
                  onCheckedChange={(checked) => setAiGenerated((prev) => ({ ...prev, top: checked }))}
                />
              </div>
            </div>
            <ImageUpload
              onImageSelect={(file) => handleImageSelect(file, "top")}
              disabled={aiGenerated.top}
            />
            <Input
              placeholder="Descripción de la prenda... (opcional)"
              value={descriptions.top}
              onChange={(e) => setDescriptions((prev) => ({ ...prev, top: e.target.value }))}
            />
          </div>

          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-gray-700">Parte inferior</label>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">Generado por IA</span>
                <Switch
                  checked={aiGenerated.bottom}
                  onCheckedChange={(checked) => setAiGenerated((prev) => ({ ...prev, bottom: checked }))}
                />
              </div>
            </div>
            <ImageUpload
              onImageSelect={(file) => handleImageSelect(file, "bottom")}
              disabled={aiGenerated.bottom}
            />
            <Input
              placeholder="Descripción de la prenda... (opcional)"
              value={descriptions.bottom}
              onChange={(e) => setDescriptions((prev) => ({ ...prev, bottom: e.target.value }))}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 pt-6">
          <button
            onClick={() => setAccessories((prev) => [...prev, { description: "" }])}
            className="flex flex-col items-center gap-1 rounded-lg border border-dashed border-gray-300 p-3 text-gray-500 transition-colors hover:border-gray-400 hover:text-gray-700"
          >
      ...
      {accessories.length > 0 && (
        <div className="space-y-3">
          <label className="text-sm font-medium text-gray-700">Accesorios</label>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {accessories.map((accessory, index) => (
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">Accesorio {index + 1}</span>
                  <button
                    onClick={() => setAccessories((prev) => prev.filter((_, i) => i !== index))}
                ...
                <ImageUpload
                  onImageSelect={(file) => {
                    setAccessories((prev) =>
                      prev.map((acc, i) => (i === index ? { ...acc, file } : acc))
                    );
                  }}
                />
                <Input
                  ...
                  onChange={(e) =>
                    setAccessories((prev) =>
                      prev.map((acc, i) => (i === index ? { ...acc, description: e.target.value } : acc))
                    )
                  }
                />
              </div>
      ...
      <div className="space-y-3">
        ...
        <div className="overflow-x-auto md:overflow-visible -mx-1 md:mx-0">
          <div className="flex gap-2 md:gap-3 px-1 py-1 md:p-1">
            ...
            return (
              ...
              >
                <img
                  src={imagePath}
                  alt={label}
                  className="h-36 w-28 md:h-44 md:w-36 object-cover flex-shrink-0"
                />
