...
const generationTips = [
  ...
  "La prenda debe estar bien visible",
];
...
export default function SinModeloPage() {
  ...
  const generateSingleImage = async (imageUrl: string, userId: string): Promise<string | null> => {
    const response = await fetch("/api/generate/no-model", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ images: [imageUrl], description, size: selectedSize }),
    });
    ...
    const images = await pollForResult(requestId, "sin-modelo", userId);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const uploadedUrl = await uploadImage("sin-modelo", "upload", userId, imageFile);
      if (!uploadedUrl) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-900">Sin modelo</h2>
        <p className="text-sm text-gray-500">Genera imágenes de prendas sin modelo</p>
      </div>
